defineClass ProjectEditor morph fileName project imagesFolder soundsFolder stage scripter library viewer viewerWidth tabs leftItems rightItems rightItemsRow2 stealthSlider title connectorToggle fpsReadout lastFrameTime frameCount sessionID sessionStartTime lastActivityCheck editCount
...
method openProject ProjectEditor projectData projectName {
  ...
  startSession this projectName
}

method startSession ProjectEditor projName {
  // Log the start of a new editing session when in the browser.

  if ('Browser' == (platform)) {
	timestamp = (time)
	sessionID = (join '' (at timestamp 1) '-' (at timestamp 2)) // seconds-usecs
	sessionStartTime = (at timestamp 1) // seconds at start
    logSessionStart sessionID projName
  }
}
...
method step ProjectEditor {
  if ('Browser' == (platform)) {
	...
	if (((msecsSinceStart) - lastActivityCheck) > 60000) {
	  if (and (notNil sessionStartTime) (editCount > 0)) {
		sessionSecs = ((first (time)) - sessionStartTime) // since start of session
		logSessionActivity sessionID sessionSecs editCount
		editCount = 0
	  }
	  lastActivityCheck = (msecsSinceStart)
	}
  }
...
method incrementEditCount ProjectEditor {
  editCount += 1
}
...
method pageResized ProjectEditor {
  ...
  viewerWidth = (max viewerWidth (350 * (global 'scale')))
